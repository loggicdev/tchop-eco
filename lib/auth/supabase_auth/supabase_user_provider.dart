...
class TxopEcoSupabaseUser extends BaseAuthUser {
  TxopEcoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> txopEcoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TxopEcoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
